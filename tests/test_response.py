...
class TestResponse(unittest.TestCase):
    ...
    def test_1_valid_no_country_code(self):
        print_color('Testing valid VAT numbers (without specifying the country code)...', colors.WARN)
    ...
    def test_2_invalid(self):
        print_color('\nTesting invalid VAT numbers (without specifying the country code)...', colors.WARN)
    ...
    def test_3_bypass_ratelimit(self):
        print_color('\nTesting ratelimit bypass...', colors.WARN)
        ...
        self.vies_api = Vies(bypass_ratelimit=True)
        response = self.vies_api.request(vat_number)
        print('%s\n' % response)
        self.assertEqual(response.is_valid, True)
